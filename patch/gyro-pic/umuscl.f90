...
subroutine mag_unsplit(uin,gravin,flux,emfx,emfy,emfz,tmp,dx,dy,dz,dt,ngrid)
  ...
  if (guiding_center) then
     ! Compute & store guiding center terms to be interpolated
     call guiding_center_gradients(qin,dq,gc_gradients)
  end if
...
!###########################################################
!###########################################################
subroutine guiding_center_gradients(q,dq,gcgrad)
! Computing the gradients necessary for the guiding center approximation
! for particles. This requires grad(P), grad(\hat{B}), grad(u).
! The requisite terms will be stored on the grid. Field advection term \bhat\bhat:\grad(u),
! and \bhat\cdot\grad(P)/\rho. More to come later.
   use amr_parameters
   use hydro_parameters
   use const
   implicit none
 ...
 end subroutine guiding_center_gradients
